...
type MenuItem = 'dashboard' | 'easyberry' | 'polling' | 'setting' | 'logout'
...
export default function Menu({ onSelect }:{ onSelect?: (m:MenuItem)=>void }){
  ...
  useEffect(()=>{
    ...
    else if(p.startsWith('/polling')) setActive('polling')
  ...
  function handleClick(item: MenuItem){
    ...
    if(item === 'polling') navigate('/polling')
    ...
    <aside className="app-sidebar" role="navigation" aria-label="Main menu">
      <div className="sidebar-top">
        ...
        <div className="sidebar-items">
          ...
          <button className={`sidebar-btn ${active==='polling'?'active':''}`} onClick={()=>handleClick('polling')}>Polling</button>
